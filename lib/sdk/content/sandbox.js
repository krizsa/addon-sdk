...
const WorkerSandbox = Class({
  ...
  initialize: function WorkerSandbox(worker, window) {
    ...
    // Use expanded principal for content-script always, if the content is a
    // regulat web content, to prevent leaking out content-script only objects.
    // (can be opted-out with security-membrane-for-content-script permission
    // flag for now)
    ...
    // domain principal.
    //
    // Also if we attach to a system window, expanded principal would be
    // meaningless.
